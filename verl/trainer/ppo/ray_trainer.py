...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.SQL_PLAN_GRPO:
        # Special handling for SQL-Plan GRPO with separate reward components
        grpo_calculation_mask = data.batch["response_mask"]
        ...
        advantages, returns, sql_advantages, plan_advantages = core_algos.compute_sql_plan_grpo_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=grpo_calculation_mask,
            index=data.non_tensor_batch["uid"],
            norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,
        ...
        data.batch["sql_advantages"] = sql_advantages
        data.batch["plan_advantages"] = plan_advantages
    else:
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if "sql_advantages" in batch.batch:
                    del batch.batch["sql_advantages"]
                if "plan_advantages" in batch.batch:
                    del batch.batch["plan_advantages"]
